...
class Preprocessor:
        ...
        def fit_outliers(self, rows: Iterable[Dict[str, Any]]) -> None:
                ...
                numeric_fields = [
                        f for f, r in self.schema.items() if r.get("type") in ("int", "integer", "float", "number")
                ]
        ...
        def transform(self, row: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if "applied_credit_limit" in row:
                        processed["credit_bucket"] = self.bucketizer.bucket_credit_limit(row.get("applied_credit_limit"))

                return processed

        def run(self, input_json: str, output_json: str) -> None:
                """Run preprocessing over a JSONL file (one JSON object per line)."""
                rows: List[Dict[str, Any]] = [json.loads(line) for line in open(input_json, "r")]
